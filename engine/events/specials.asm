...
RespawnOneOffs:
	...
	eventflagreset EVENT_ARTICUNO_NEST_ARTICUNO
.CaughtArticuno

	ld de, ENGINE_PLAYER_CAUGHT_ZAPDOS
	farcall EngineFlagAction
	jr nz, .CaughtZapdos
	eventflagreset EVENT_ZAPDOS_NEST_ZAPDOS
.CaughtZapdos

	ld de, ENGINE_PLAYER_CAUGHT_MOLTRES
	farcall EngineFlagAction
	jr nz, .CaughtMoltres
	eventflagreset EVENT_MOLTRES_NEST_MOLTRES
.CaughtMoltres

	ld de, ENGINE_PLAYER_CAUGHT_MEWTWO
	farcall EngineFlagAction
	jr nz, .CaughtMewtwo
	eventflagreset EVENT_CERULEAN_CAVE_B1F_MEWTWO
.CaughtMewtwo
